...
def check_availability(username: str = Query(None), email: str = Query(None), db: Session = Depends(get_db)):
    ...
    try:
        ...
        return {"success": True, "data": {"username_exists": False, "email_exists": False}}
...
def register(body: RegisterBody, db: Session = Depends(get_db)):
    try:
        ...
        return {"success": False, "data": None, "message": f"数据库错误，请确保 backend 目录可写且 ai_quant.db 未被占用: {str(e)[:80]}", "code": 500}
...
def login(body: LoginBody, db: Session = Depends(get_db)):
    try:
        ...
        return {"success": False, "data": None, "message": f"数据库错误: {str(e)[:80]}", "code": 500}
